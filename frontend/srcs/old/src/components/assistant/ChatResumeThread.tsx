...
function ResumeSnapshot({ resume }: { resume: any }) {
  return (
    <div className="rounded-2xl border border-brand-100 bg-brand-50/50 p-4">
      ...
      {resume?.professional_experience?.slice(0, 2).map((exp: any, i: number) => <div key={i} className="border-t border-brand-100 pt-3 mt-3"><div className="text-xs font-bold text-gray-900">{exp.role}</div><div className="text-[11px] text-gray-500">{exp.company} · {exp.start_date} – {exp.end_date}</div><ul className="mt-2 space-y-1 text-[11px] leading-4 text-gray-700">{(exp.responsibilities || []).slice(0, 3).map((b: string, j: number) => <li key={j}>• {b}</li>)}</ul></div>)}
    </div>
  );
}
