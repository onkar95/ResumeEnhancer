...
export default function ReviewPage() {
  ...
  const [view, setView] = useState<"compare" | "tailored" | "original">(
    "compare",
  );
  ...
  async function load() {
    ...
    try {
      setLoading(true);
      const data = await fetchRun(resolved);
      saveRunId(resolved);
      setRun(data);
    } catch (e) {
      console.error(e);
  ...
  async function refresh() {
    ...
    setRefreshing(true);
    try {
      setRun(await fetchRun(resolved));
    } finally {
      setRefreshing(false);
    }
  }
    ...
    <div className="overflow-x-auto">
      <div className="min-w-[640px] lg:min-w-0">
        <EditableResumeRenderer
          ...
          onEdited={refresh}
          showChanges={
            original != null || original != undefined ? showChanges : false
          }
        />
      </div>
    </div>
  ...
  return (
    <div className="min-h-full bg-gray-50">
      <div className="sticky top-0 z-30 border-b border-gray-200 bg-white">
        <div className="mx-auto flex max-w-[1500px] items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:px-8">
          ...
          <div className="min-w-0 text-center">
            ...
            <p className="truncate text-xs text-gray-500">
              {run.tailored_resume?.name || "Resume"}
            </p>
          </div>
          <div className="flex shrink-0 items-center gap-2">
            <span className="hidden rounded-full bg-brand-50 px-2.5 py-1 text-[10px] font-semibold text-brand-700 sm:inline-flex">
              {run.finalized ? "Finalized" : "Draft"}
            </span>
            <button
              onClick={() =>
                navigate(`/review/${resolved}/assistant/suggestions`)
              }
        ...
        <div className="mx-auto max-w-[1500px] border-t border-gray-100 px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-3">
            {(["compare", "tailored", "original"] as const).map((v) => (
              <button
                key={v}
                onClick={() => setView(v)}
                className={`relative h-11 text-xs font-semibold sm:text-sm ${view === v ? "text-brand-700" : "text-gray-500"}`}
              >
                {v === "compare"
                  ? "Compare"
                  : v === "tailored"
      ...
      <div className="mx-auto max-w-[1500px] px-4 py-5 sm:px-6 lg:px-8">
        ...
        <div className="mt-5 flex flex-col gap-2 rounded-xl border border-brand-100 bg-brand-50 px-4 py-3 text-sm sm:flex-row sm:items-center sm:justify-between">
          <span className="text-brand-900">
            Review the AI changes, make inline edits, then finalize when ready.
          </span>
          <div className="flex gap-2">
            <a
              href={getExportPdfUrl(resolved!)}
              className="btn-secondary px-3 py-2 text-xs"
            >
              PDF
            </a>
            <a
              href={getExportDocxUrl(resolved!)}
              className="btn-secondary px-3 py-2 text-xs"
            >
              Word
            </a>
            {run.finalized ? (
              <span className="rounded-lg bg-brand-600 px-3 py-2 text-xs font-semibold text-white">
                Finalized
              </span>
            ) : (
              <button
            ...
            <div className="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-gray-200 bg-white px-4 py-3 shadow-sm">
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs text-gray-500">
                  {showChanges
                    ? "Added text is green and removed text is red."
                    : "Showing the tailored resume normally."}
                </span>
              </div>
              {showChanges && (
                <div className="flex items-center gap-4 text-[11px] font-medium text-gray-500">
                  <span className="inline-flex items-center gap-1.5">
                    <span className="h-2.5 w-2.5 rounded bg-green-200" /> Added
                    / changed
                  </span>
                  <span className="inline-flex items-center gap-1.5">
                    <span className="h-2.5 w-2.5 rounded bg-red-200" /> Removed
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="mt-5 grid gap-6 xl:grid-cols-2">
              ...
              <section className="min-w-0">
                ...
                <ResumePanel
                  ...
                  original={showChanges ? run.parsed_resume : undefined}
                />
              </section>
            </div>
          </>
        ...
        {view === "tailored" && (
          <div className="mx-auto mt-6 max-w-4xl">
            <ResumePanel resume={run.tailored_resume} />
          </div>
        )}
        {view === "original" && (
          <div className="mx-auto mt-6 max-w-4xl">
        ...
        <div className="mt-8 grid gap-6 xl:grid-cols-2">
          <GapAnalysis gapAnalysis={run.gap_analysis} />
          <ValidationPanel validation={run.validation_result} />
        </div>
      </div>
    </div>
  );
